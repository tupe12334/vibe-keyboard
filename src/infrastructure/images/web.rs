...
pub fn generate_web_image() -> RgbImage {
    ...
    for &y in &[38u32, 50, 62] {
        let half_w = (((35.0f64).powi(2) - ((y as f64) - 50.0).powi(2)).max(0.0).sqrt()) as u32;
        if half_w > 0 {
            let x_start = 50u32.saturating_sub(half_w);
            let x_end   = (50 + half_w).min(99);
            for x in x_start..=x_end { img.put_pixel(x, y, dark); }
        }
    }

    // Vertical central meridian
    for y in 15u32..=85 { img.put_pixel(50, y, dark); }
